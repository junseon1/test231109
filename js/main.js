$(document).ready(function(){
  ...
  if($(window).width() >=1280){
    ...
    $('.sec6 .center .business>ul>li').hover(function(){
    ...
    $('nav > ul > li > a').click(function(){
      // a태그의 다음 객체인 ul.sub가 slideToglle됨
      $(this).next().slideToggle();
      // 클릭하지 않은(.not(this)) a객체의 ul.sub는 slideUp됨
      $('nav > ul > li > a ').not(this).next().slideUp();
      // 클릭한 주메뉴에 active 클래스를 추가하거나 제거
      $(this).toggleClass('active');
      // 클릭하지 않은 주메뉴(not(this))의 active제거
      $('nav > ul > li > a ').not(this).removeClass('active');
      return false;
    });
    // header의 메뉴 버튼을 클릭하면 내비게이션 나타나고, 
    // 다시 클릭하면 내비게이션 들어감
    let menu_sw=0;
  ...
  minimize_header();
    ...
    // 스크롤 내릴때마다 section안의 자식 콘텐츠들이 서서히 나타나는 애니메이션
